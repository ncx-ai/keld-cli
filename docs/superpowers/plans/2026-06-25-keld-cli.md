...
**Tech Stack:** Python 3.12, Typer (CLI), httpx (HTTP), rich (output), tomlkit (Codex TOML round-trips), stdlib `tomllib`/`json`. Packaged with uv + hatchling; pytest for tests.

## Global Constraints

- Python `>=3.12`. Source layout: `src/keld/`. Console entry point: `keld = "keld.cli:main"`.
- Dependencies limited to: `typer>=0.12`, `httpx>=0.27`, `rich>=13`, `tomlkit>=0.13`. Dev: `pytest>=8`. Do not add others without updating this constraint.
...
- Every task ends by running its tests (`uv run pytest <path> -v`) and committing.
...
dependencies = ["typer>=0.12", "httpx>=0.27", "rich>=13", "tomlkit>=0.13"]
...
`tests/test_paths.py`:
```python
from keld.paths import keld_home, auth_path, manifest_path, hook_path, api_base


def test_keld_home_honors_env(keld_home):
    assert keld_home == __import__("keld.paths", fromlist=["keld_home"]).keld_home()
    assert auth_path().name == "auth.json"
    assert manifest_path().name == "manifest.json"
    assert hook_path().name == "keld-context.py"


def test_api_base_default(monkeypatch):
    monkeypatch.delenv("KELD_API_URL", raising=False)
    assert api_base() == "https://atlas.keld.co"
...
Run: `uv run pytest -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'keld'`.

- [ ] **Step 4: Implement the package**

`src/keld/__init__.py`: `__version__ = "0.1.0"`
...
Run: `uv run pytest -v`
Expected: PASS (3 tests). Also verify `uv run keld --help` lists all subcommands.
...
Run: `uv run pytest tests/config/test_manifest.py -v`
...
Run: `uv run pytest tests/config/test_merge_json.py -v`
...
Run: `uv run pytest tests/config/test_merge_toml.py -v`
...
Run: `uv run pytest tests/tools/test_claude.py -v`
...
Run: `uv run pytest tests/tools/test_gemini.py -v`
...
Run: `uv run pytest tests/tools/test_codex.py -v`
...
class AtlasClient:
    ...
    def fetch_hook(self, endpoint: str, ingest_token: str) -> bytes:
        try:
            resp = httpx.get(f"{endpoint.rstrip('/')}/v1/tool-context/hook.py",
                             params={"token": ingest_token}, timeout=30.0,
...
Note: `fetch_hook` reuses the client's transport so tests can mock it; in production the transport is the real default.
...
Run: `uv run pytest tests/auth/test_device_flow.py -v`
...
Run: `uv run pytest tests/config/test_writer.py -v`
...
Run: `uv run pytest tests/commands/test_login.py -v`
...
Run: `uv run pytest tests/commands/test_setup.py -v`
...
Run: `uv run pytest tests/commands/test_status.py -v`
...
Run: `uv run pytest tests/commands/test_uninstall.py -v`
...
Run: `uv run keld --help` and `uv run keld status` (with a temp `KELD_HOME`).
...
- **Spec coverage:** login/logout/whoami (T16), setup (T17), status/doctor (T18), uninstall (T19); provider-agnostic adapters + registry (T2, T7–T10); managed-block + manifest (T3, T5, T6); browser device flow + lazy auth (T11–T13); hook download from backend (T12, T14); atomic writes + backups (T15); uv/hatchling packaging (T1); Gemini in v1 (T8). Backend endpoints are out of scope per spec §10 and mocked in tests.
